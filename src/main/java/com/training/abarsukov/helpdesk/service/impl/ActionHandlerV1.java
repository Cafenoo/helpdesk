...
public class ActionHandlerV1 implements ActionHandler {
  ...
  private static final Map<Role, List<Action>> DRAFT_COLUMN = Map.of(
      EMPLOYEE, List.of(SUBMIT, CANCEL),
      MANAGER, List.of(SUBMIT, CANCEL));

  private static final Map<Role, List<Action>> NEW_COLUMN = Map.of(
      MANAGER, List.of(APPROVE, DECLINE, CANCEL));

  private static final Map<Role, List<Action>> APPROVED_COLUMN = Map.of(
      ENGINEER, List.of(ASSIGN_TO_ME, CANCEL));

  private static final Map<Role, List<Action>> DECLINED_COLUMN =
      DRAFT_COLUMN;

  private static final Map<Role, List<Action>> CANCELLED_COLUMN =
      Collections.emptyMap();

  private static final Map<Role, List<Action>> IN_PROGRESS_COLUMN = Map.of(
      ENGINEER, List.of(DONE));

  private static final Map<Role, List<Action>> DONE_COLUMN =
      Collections.emptyMap();

  private static final Map<State, Map<Role, List<Action>>> ACTION_TABLE = Map.of(
  ...
  public List<Action> getPossibleActions(State state) {
    final Role userRole = userService.getUser()
        .getRole();
  ...
  public void transitState(Ticket ticket, Action action) {
    ...
    final Map<Action, Runnable> transitionInstruction = Map.of(
    ...
    transitionInstruction.get(action)
        .run();
  }
  ...
  private void handleCancel(Ticket ticket) {
    ...
    final Role userRole = userService.getUser()
        .getRole();
    final Map<Role, Runnable> instructionToSendEmailInCanceled = Map.of(
