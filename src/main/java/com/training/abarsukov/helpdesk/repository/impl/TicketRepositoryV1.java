...
@Repository
public class TicketRepositoryV1 extends AbstractRepository<Ticket, Long>
    implements TicketRepository {

  @Override
  public List<Ticket> findAll(
      Integer page, Integer pageSize, Boolean isPersonal, String keyword, User user) {
    final Map<Boolean, Supplier<String>> conditionsMap =
        Map.of(
    ...
    final Map<String, Object> parameters = new HashMap<>(Map.of(Parameters.USER_ID, user.getId()));
  ...
  public Optional<Ticket> findById(Long id, User user) {
    final String query =
        Queries.SELECT_TICKETS
            + WHERE
            + Queries.CONDITION_TO_GET_TICKET_BY_ID
            + AND
            + Queries.MAP_OF_CONDITIONS_TO_GET_TICKETS.get(user.getRole());

    final Map<String, Object> parameters =
        Map.of(Parameters.TICKET_ID, id, Parameters.USER_ID, user.getId());
  ...
  public Optional<Ticket> findByIdToEditTicket(Long id, User user) {
    final String query =
        Queries.SELECT_TICKETS
            ...
            + Queries.CONDITION_TO_GET_TICKET_BY_ID
            + AND
            + Queries.CONDITION_TO_EDIT_TICKET;
  ...
  public Optional<Ticket> findByIdToSaveFeedback(Long id, User user) {
    final String query =
        Queries.SELECT_TICKETS
            ...
            + Queries.MAP_OF_CONDITIONS_TO_GET_TICKETS.get(user.getRole())
            + AND
            + Queries.CONDITION_TO_SAVE_FEEDBACK;
  ...
  public static final class Queries {
    ...
    private static final String CONDITION_TO_GET_TICKET_BY_ID =
        format("(t.id = :{0})", Parameters.TICKET_ID);

    private static final String CONDITION_TO_SEARCH_BY_KEYWORD =
        format(
            "("
    ...
    private static final String CONDITION_TO_GET_TICKETS_OF_EMPLOYEE =
        format("(t.owner.id = :{0})", Parameters.USER_ID);

    private static final String CONDITION_TO_GET_TICKETS_OF_MANAGER =
        format(
            "(t.owner.id = :{7}"
                + " or t.owner.role = ''{0}''"
                + " and t.state = ''{1}''"
    ...
    private static final String CONDITION_TO_GET_TICKETS_OF_ENGINEER =
        format(
            "(t.owner.role in(''{0}'', ''{1}'')"
                + " and t.state = ''{2}''"
                + " or t.assignee.id = :{5}"
    ...
    private static final String CONDITION_TO_GET_PERSONAL_TICKETS_OF_EMPLOYEE =
        format("(t.owner.id = :{0})", Parameters.USER_ID);

    private static final String CONDITION_TO_GET_PERSONAL_TICKETS_OF_MANAGER =
        format(
            "(t.owner.id = :{0} or t.approver.id = :{0} and t.state = ''{1}'')",
            Parameters.USER_ID, APPROVED);

    private static final String CONDITION_TO_GET_PERSONAL_TICKETS_OF_ENGINEER =
        format("(t.assignee.id = :{0})", Parameters.USER_ID);

    public static final Map<Role, String> MAP_OF_CONDITIONS_TO_GET_TICKETS =
        Map.of(
    ...
    public static final Map<Role, String> MAP_OF_CONDITIONS_TO_GET_PERSONAL_TICKETS =
        Map.of(
    ...
    private static final String CONDITION_TO_EDIT_TICKET =
        format("(t.owner.id = :{0})", Parameters.USER_ID)
            + AND
            + format("(t.state = ''{0}'')", State.DRAFT);

    private static final String CONDITION_TO_SAVE_FEEDBACK =
        format("(t.state = ''{0}'')", State.DONE)
            + AND
            + format("(t.owner.id = :{0})", Parameters.USER_ID);
  }
